$(function(){
	...
	$('#DataTables_table_paginate .paginate_button').on( 'click', function () {
		...
		window.location.href = "message-list-page.html?"+requrl+"pageNo="+val;
	});
})
...
function examine_info(title,url,id,w,h,pageNo){
	url = url + "?id="+id+"&pageNo="+pageNo;
	layer_show(title,url,w,h);
}
...
function del_info(id,pageNo){
	layer.confirm('确认要删除吗？',function(index){
		window.location.href = "message-delete.html?pageNo="+pageNo+"&id="+id;
	});
}
...
function datadel(pageNo){
	...
	layer.confirm('确认要删除这些信息吗？',function(index){
		window.location.href = "message-deletes.html?pageNo="+pageNo+"&ids="+ids;
	});
}
