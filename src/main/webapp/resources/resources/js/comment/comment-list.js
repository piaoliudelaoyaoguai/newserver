$(function(){
	...
	$('#DataTables_table_paginate .paginate_button').on( 'click', function () {
		...
		window.location.href = "comment-list-page.html?"+requrl+"pageNo="+val;
	});
})
...
function del_info(id,pageNo){
	layer.confirm('确认要删除吗？',function(index){
		window.location.href = "comment-delete.html?pageNo="+pageNo+"&id="+id;
	});
}
...
function datadel(pageNo){
	...
	layer.confirm('确认要删除这些信息吗？',function(index){
		window.location.href = "comment-deletes.html?pageNo="+pageNo+"&ids="+ids;
	});
}
