$(function(){
	...
	$('#DataTables_table_paginate .paginate_button').on( 'click', function () {
		...
		window.location.href = "promotion-list-page.html?pageNo="+val;
	});
})
...
function del_info(id,pageNo){
	layer.confirm('确认要删除吗？',function(index){
		window.location.href = "promotion-delete.html?pageNo="+pageNo+"&id="+id;
	});
}

function scorelist_info(title,url,user_id,w,h,pageNo){
	url = url + "?user_id="+user_id+"&pageNo="+pageNo;
	layer_show_max(title,url,w,h);
}

function jumpupdatescore_info(title,url,user_id,w,h,pageNo){
	url = url + "?user_id="+user_id+"&pageNo="+pageNo;
	layer_show(title,url,w,h);
}


function redpacket_info(title,url,user_id,w,h,pageNo){
	url =program + url + "?user_id="+user_id+"&pageNo="+pageNo;
	layer_show_max(title,url,w,h);
}
...
function datadel(pageNo){
	...
	layer.confirm('确认要删除这些信息吗？',function(index){
		window.location.href = "promotion-deletes.html?pageNo="+pageNo+"&ids="+ids;
	});
}
